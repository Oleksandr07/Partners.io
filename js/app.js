...
// }
var link=$(this).attr('href');if(link.charAt(0)==='/'){link=link.slice(1,link.length)}if($(link).length){var posTop=$(link).offset().top-parseInt($(link).css('padding-top'))-$('.header').innerHeight();$('html, body').animate({'scrollTop':posTop+'px'},500);if(window.matchMedia('(max-width: 1199px)').matches){$('.menu-toggle').removeClass('active');$('.header-modal').removeClass('show')}}});/* - - - - - - - - - - - - - -       rebuild header       - - - - - - - - - - - - - - - */function rebuildHeader(){var $button=$('.header-button').detach();var $lang=$('.lang').detach();if(window.matchMedia('(max-width: 1199px)').matches){$('.header-modal').append($lang);$('.header-modal').append($button)}else{$('.header-right').prepend($button);$('.header-right').prepend($lang)}}rebuildHeader();$(window).on('resize',function(){rebuildHeader()});/* - - - - - - - - - - - - - -       fixed header       - - - - - - - - - - - - - - - */function fixedHeader(){var top=$(window).scrollTop();if(top>0){$('.header').addClass('fixed')}else{$('.header').removeClass('fixed')}}fixedHeader();$(window).on('scroll',function(){fixedHeader()});setTimeout(function(){$('.header').css('transform','none')},1000);/* - - - - - - - - - - - - - -       menu toggle       - - - - - - - - - - - - - - - */$(document).on('click','.menu-toggle',function(){var $this=$(this);if(!$this.hasClass('active')){$this.addClass('active');$('.header-modal').addClass('show')}else{$this.removeClass('active');$('.header-modal').removeClass('show')}});/* - - - - - - - - - - - - - -       submenu toggle       - - - - - - - - - - - - - - - */$(document).on('click','.top-menu .icon, .top-menu li span:not(.icon)',function(){if(window.matchMedia('(max-width: 1199px)').matches){var $parent=$(this).closest('li');if(!$parent.hasClass('show')){$parent.addClass('show');$parent.find('ul').slideDown(300)}else{$parent.removeClass('show');$parent.find('ul').slideUp(300)}}});$(window).on('resize',function(){if(window.matchMedia('(min-width: 1200px)').matches){$('.top-menu li').removeClass('show');$('.top-menu ul').css('display','')}});/* - - - - - - - - - - - - - -       lang toggle       - - - - - - - - - - - - - - - */$(document).on('click','.lang-title',function(){if(window.matchMedia('(max-width: 1199px)').matches){var $parent=$(this).closest('.lang');if(!$parent.hasClass('show')){$parent.addClass('show')}else{$parent.removeClass('show')}$(document).click(function(e){if($(e.target).closest('.lang').length){return}$('.lang').removeClass('show')})}});$(window).on('resize',function(){if(window.matchMedia('(min-width: 1200px)').matches){$('.lang').removeClass('show')}});/* - - - - - - - - - - - - - -       account toggle       - - - - - - - - - - - - - - - */// $( document ).on( 'click', '.js-account', function( e ) {
// 	e.preventDefault();
